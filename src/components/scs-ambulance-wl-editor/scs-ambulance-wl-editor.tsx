...
@Component({
  tag: 'scs-ambulance-wl-editor',
  styleUrl: 'scs-ambulance-wl-editor.css',
  shadow: true,
})
export class ScsAmbulanceWlEditor {
  @Prop() entryId: string;
  ...
  private handleCondition(ev: InputEvent) {
    ...
    const condition = (this.conditions || []).find(c => c.code === code);
    if (!condition) return;
    this.entry.condition = Object.assign({}, condition);
    this.entry.estimatedDurationMinutes = condition.typicalDurationMinutes ?? this.entry.estimatedDurationMinutes;
  ...
  private async updateEntry() {
    try {
      ...
      const response = this.entryId === '@new' ? await api.createWaitingListEntry(this.ambulanceId, this.entry) : await api.updateWaitingListEntry(this.ambulanceId, this.entryId, this.entry);
      if (response.status < 299) {
